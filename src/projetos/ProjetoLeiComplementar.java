...
/**
 * Classe filha de Projeto, permite criar um objeto do tipo Projeto de Lei Complementar.
 * @author Edimar Junior, Gabriel Brandao, Gabriel Medeiros, Ruan Gomes.
 *
 */
public class ProjetoLeiComplementar extends Projeto implements Serializable{
	...
	/**
	 * Construtor da classe filha ProjetoLeiComplementar.
	 ...
	 */
	public ProjetoLeiComplementar(Pessoa deputado, int ano, String codigo, String ementa, String interessesRelacionados, String url, String artigos, int numero) {
	...
	/**
	 * Retorna uma String contendo a representacao textual do projeto de lei complementar.
	 * @return String contendo a representacao textual.
	 */
	@Override
	public String toString() {
		return "Projeto de Lei Complementar" + " - " + this.codigo + " - " +  this.deputado.getDni() + " - " + this.ementa + " - " + this.artigos.replace(",", ", ") + " - " + this.situacao;
	}
	...
	public boolean votarPlenario(int total, int numeroDeputados, int aprovacoes) {
		...
		if ((Math.floor((1/2.0) * total + 1) <= aprovacoes)){
			this.tramitacao.add(String.format("APROVADO (%s)", this.primeiraLetraMaiuscula(this.getLocal())));
